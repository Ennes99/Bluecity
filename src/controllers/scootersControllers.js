...
scooterController.Creates = (req,res)=>{
  const {userId, boxId, createAt, updateAt} = req.body;
  Scooter.create({userId : userId, boxId :  boxId, createAt : createAt, updateAt : updateAt})
    .then(scooters => {
        if (scooters.id){
...
scooterController.Updates = (req,res)=>{
  const {id} = req.params;
  const {userId, boxId, createAt, updateAt}=req.body;
  Scooter.update({userId : userId, boxId : boxId, createAt : createAt, updateAt : updateAt  },
      { where: {id:id} })
    .then(scooters => {
      const scootersString = JSON.stringify(scooters);
      const scootersObject = JSON.parse(scootersString);
